        ...
        <div>
            ...
            >
                <div className='mt-12 md:mt-24 ml-4 md:ml-24'>
                    <p id="passion" className="text-2xl md:text-3xl font-sans font-bold text-gray-800 mb-2 md:mb-3 mt-6 md:mt-6">FOLLOW YOUR PASSION</p>
                    <p id="stem" className="text-base md:text-lg text-gray-800 mb-3 md:mb-5">Science Technology Engineering Mathematics</p>
                    <div className="relative mb-3 md:mb-6">
                        ...
                        >
                            <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-4.35-4.35m2.85-3.65A7.5 7.5 0 1112 4.5a7.5 7.5 0 017.5 7.5z" />
                        </svg>
                    </div>
                </div>
                <div className='options-container absolute top-0 right-0 mt-24 mr-4 md:mr-36'>
                    ...
                    {Options.map((option, index) => (
                        <Link to={option.link} key={index}>
                            <div className={`option-item text-left text-lg md:text-3xl font-sans text-gray-800 mb-2 mr-72 ${optionIndex === index ? 'font-bold' : ''}`}>
                                {option.text}
                            </div>
                        </Link>
                    ))}
            ...
            <div className='text-center'>
                <p className="text-sm text-gray-800 mt-5">Discover Bachelor's Program in Science and Engineering</p>
                <div className='mt-5 mb-5 flex flex-wrap justify-center gap-4'>
                    {Options.map((option, index) => (
                        <Link to={option.link} key={index}>
                            <button className='h-28 w-52 bg-[#cbd5e1] border border-black rounded-lg flex flex-col items-center justify-center'>
                                <img src="./science.png" alt={option.text} className='h-16 rounded-lg'/>
                                <span className="hover:underline text-gray-800 font-medium hover:text-orange-600 mt-2">{option.text}</span>
                            </button>
                        </Link>
                    ))}
                </div>
            ...
            <div className='mb-8 mt-8 flex flex-wrap justify-center gap-4'>
                {cities.slice(startIndex, startIndex + citiesPerPage).map((city, index) => (
                    <Link to="#" key={index}>
                        <button className='h-28 w-44 border border-black rounded-lg flex flex-col items-center justify-center'>
                            <img src={city.img} alt={city.name} className='h-16 rounded-lg'/>
                            <span className="hover:underline text-gray-800 font-medium hover:text-orange-600 mt-2">{city.name}</span>
                        </button>
                    </Link>
                ))}
            </div>
